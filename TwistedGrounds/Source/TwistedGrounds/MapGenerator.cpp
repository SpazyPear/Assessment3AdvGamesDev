...
void AMapGenerator::Tick(float DeltaTime)
{
	...
	if (bRegenerateMap) {
		...
		ClearMaps();
		UpdateValues();
		CheckSurrounding(FVector(0, 0, 0), false);
	}
}
...
{
	CheckSurrounding(Position, true);
}

void AMapGenerator::CheckSurrounding(FVector Position, bool bOnServer)
{
	...
	for (int i = 0; i < ChunkRadius * ChunkRadius; i++) {
		...
		Map->SetMapValues(ChunkWidth, ChunkHeight, ChunkGridSize, PerlinScale, PerlinRoughness, PerlinOffset, OffsetX, OffsetY);
		if (bOnServer) {
			Map->NetMulticastGenerateMap();
		}
		else {
			Map->ThreadedGenerateMap();
		}
	}
}
